...
class Kernel:
    ...
    _sys_info = [_inputHistory, _outputHistory, _inputStack]

    def __init__(self, sessionStore=None, sess_lifetime=1800):
        ...
        self._sess_lifetime = sess_lifetime

        # set up the sessions   
        if sessionStore is not None:
            self._sessions = sessionStore # 1th session param
        else:     
            self._sessions = {}
        self._addSession(self._globalSessionID) # 2th session param

        # Set up the bot predicates
        self._botPredicates = {}
        self.setBotPredicate("name", "小胖")
    ...
    def getPredicate(self, name, sessionID=_globalSessionID):
        ...
        """
        if name == "name":
            res = db.find_request(sessionID)
            if isinstance(res, dict) and name in res:
                return res["name"]
            elif name in self._sessions[sessionID]:
        ...
        if name == "name":
            try:
                ...
                if isinstance(res, dict):
                    res[name] = value
                    db.update_request(sessionID, set={name: value})
                else:
                    db.insert_request({"sid": sessionID, name: value})
            except:
                pass
    ...
    def _addSession(self, sessionID):
        ...
        if sessionID in self._sessions:
            self._sessions[sessionID]["time"] = time.time()
            return
        # Create the session.
    ...
    def respond(self, input_, sessionID=_globalSessionID):
        ...
        try:
            ...
            # auto delete dead session
            time_sign = time.time()
            if len(self._sessions) > 1:
                for ses in self._sessions:
                    if ses != sessionID and time_sign-self._sessions[ses]["time"] >= self._sess_lifetime:
                        self._deleteSession(ses)
            # split the input into discrete sentences
            sentences = Utils.sentences(input_)
            # print("dubug: ", [s for s in sentences])
            finalResponse = u""
            for s in sentences:
    ...
    def _respond(self, input_, sessionID):
        ...
        if elem is None:
            ...
            response += u" "
        # print("0000", response == u"", " len:", len(response))
    ...
    # <bot>
    def _processBot(self, elem, sessionID): # 这是内置的
